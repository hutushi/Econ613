...
change <- datall %>%
  ...
  mutate(move = datent == year) %>%
  group_by(idind) %>%
  mutate(pronumb = n_distinct(profession, na.rm = T)) %>%
  mutate(empnumb = n_distinct(empstat, na.rm =T)) %>%
  ungroup() %>%
  filter(move == TRUE, pronumb >= 2 | empnumb >= 2)

z4 <- length(unique(change$idmen))

```

There are `r z4` households who migrate had at least one family member changed profession.
